...
def skipgrams(sequence, n, k, pad_left, impl=1):
    if impl == 1:
        return skipgrams_1(sequence, n, k, pad_left=pad_left)
    elif impl == 2:
        return skipgrams_2(sequence, n, k)
    else:
        raise NotImplementedError(impl)


def skipgrams_1(sequence, n, k, pad_left=False, pad_right=False, pad_symbol=None):
    ...
    # If n+k longer than sequence, reduce k by 1 and recur
        ...
        for ng in skipgrams_1(list(sequence), n, k - 1):
            yield ng
    ...
    # len(sequence) < n+k
    for ng in list(skipgrams_1(history, n, k - 1)):
        yield ng


# alternative implementation
def skipgrams_2(sequence, n, k):
